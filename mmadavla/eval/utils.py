...
class MMaDA_VLA_Server(object):
    def __init__(
        ...
        action_stats_path = os.path.join(action_stats_path, f"action_stats_{self.training_args['action_chunk_size']}chunk.json")
        self.action_stats = load_action_stats(action_stats_path)[self.benchmark] if os.path.exists(action_stats_path) else None
        print(f"{type(self.action_stats)=}")
        ...
        if os.path.exists(self.training_args["pretrained_mmada"]):
            self.tokenizer = AutoTokenizer.from_pretrained(self.training_args["pretrained_mmada"], padding_side="left")
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(os.path.join(os.sep, "ssdwork", "liuyang", "Models", "MMaDA-8B-Base"), padding_side="left")
        self.cache = cache
        if self.cache:
            ...
            cache_instance.reset_cache(prompt_length=self.mmadavla.config.vision_num_vq_tokens * 2 + self.mmadavla.config.action_num_vq_tokens + self.training_args["max_text_len"] + 9)
    ...
    def inference(
        ...
        for act, img in zip(action_sampled_ids_list, vision_sampled_ids_list):
            action = self.decode_action(act).squeeze(axis=0)
    ...
    def decode_action(self, action_ids: torch.Tensor) -> np.ndarray:
        action_ids = torch.clamp(action_ids, max=self.mmadavla.config.action_codebook_size - 1, min=0)
        ...
        action = unnormalize_action(action=action, action_stats=self.action_stats) if self.action_stats is not None else action
        if self.benchmark == "calvin":
            pass
        elif self.benchmark == "libero":
            action[..., -1] = 0 - action[..., -1]
        if not (self.benchmark in ("banana_blue_bowl", "stacking_block")):
    ...
    def batch_inference(
        ...
        gripper_images: List[Union[np.ndarray, Image.Image]],
        merged_images: List[Union[Image.Image, None]] = None,
    ) -> Tuple[List[torch.Tensor], List, List[np.ndarray]]:
        batch_size = len(task_insts)
        if merged_images is None:
            merged_images = []
            for image, gripper_image in zip(images, gripper_images):
                image = np.array(image) if isinstance(image, Image.Image) else image
                gripper_image = np.array(gripper_image) if isinstance(gripper_image, Image.Image) else gripper_image
                merged_image = merge_multiview_rgb(third_rgb=image, gripper_rgb=gripper_image)
                merged_images.append(merged_image)
        merged_image_tensors = torch.stack([image_transform(img).to(self.device) for img in merged_images])
        merged_image_tokens = self.vision_vq_model.get_code(merged_image_tensors) + len(self.prompt.tokenizer)
        vision_tokens = torch.ones((batch_size, self.mmadavla.config.vision_num_vq_tokens), dtype=torch.long, device=self.device) * self.mask_token_id
        action_tokens = torch.ones((batch_size, self.mmadavla.config.action_num_vq_tokens), dtype=torch.long, device=self.device) * self.mask_token_id
        input_ids, attention_bias = self.prompt(
            task_inst=task_insts,
            cur_image_tokens=merged_image_tokens,
            pred_image_tokens=vision_tokens,
            action_tokens=action_tokens,
        )
